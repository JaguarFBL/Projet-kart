...
function fetchLive() {
  fetch('api_data.php')
    ...
    .then(data => {
      ...
      setText('temp-piste-val', tempP); setText('ambiant-temp', tempP); setText('cond-temp-val', tempP);
      if (tempP !== '—') { const [l,c2] = tempBadge(parseInt(tempP)); setBadge('temp-piste-badge', l, c2); setBadge('ambiant-badge', l, c2); }

      setText('humid-val', humid); setText('accueil-humid', humid); setText('cond-humid-val', humid);
      ...
      // ── Session : meilleur/pire tour, nb tours, autonomie ──
      if (pct !== '—') {
        const auto = Math.round(parseInt(pct) * 0.6);
        setText('session-autonomie', auto);
      }

      // Meilleur / pire / écart — recalculé depuis data.tours
      if (data.tours) {
        const validTours = data.tours.filter(t => parseInt(t.temps) > 0);
        const nbT = validTours.length;
        setText('session-nb-tours', nbT);
        if (nbT > 0) {
          const times2 = validTours.map(t => parseInt(t.temps));
          const best2  = Math.min(...times2);
          const worst2 = Math.max(...times2);
          setText('session-best-lap',  best2);
          setText('session-worst-lap', worst2);
          const gapEl = document.getElementById('session-worst-gap');
          if (gapEl) gapEl.textContent = '+' + Math.round(worst2 - best2) + ' ms vs meilleur';
        }
      }
